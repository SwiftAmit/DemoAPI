...
open class DASwiftAlert: UIViewController {
    ...
    fileprivate func resizeAndRelayout() {
        ...
        for i in 0 ..< buttons.count {
            ...
            buttons[i].addTarget(self, action: #selector(DASwiftAlert.pressed(_:)), for: UIControlEvents.touchUpInside)
    ...
    func closeAlert(_ buttonIndex:Int){
        if userAction !=  nil {
            ...
            DASwiftAlertContext.shouldNotAnimate = true
            userAction!(isOtherButton)
            DASwiftAlertContext.shouldNotAnimate = false
        }
    ...
    open func showAlert(_ title: String) -> DASwiftAlert {
        _ =  showAlert(title, subTitle: nil, style: .none)
        return self
    }
    
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle) -> DASwiftAlert {
        _ = showAlert(title, subTitle: subTitle, style: style, buttonTitle: "OK")
        return self
        
    }
    
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> DASwiftAlert {
        _ = showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: UIColor.colorFromRGB(0xAEDEF4))
        userAction = action
        return self
    }
    
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,action: ((_ isOtherButton: Bool) -> Void)? = nil) -> DASwiftAlert {
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> DASwiftAlert {
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        ...
        if otherButtonTitle != nil && otherButtonTitle!.isEmpty == false {
            ...
            button.addTarget(self, action: #selector(DASwiftAlert.pressed(_:)), for: UIControlEvents.touchUpInside)
            button.tag = 1
            buttons.append(button)
        }
        
        resizeAndRelayout()
        if DASwiftAlertContext.shouldNotAnimate == true {
    ...
    fileprivate struct DASwiftAlertContext {
