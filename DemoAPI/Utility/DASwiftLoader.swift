...
public class DASwiftLoader: UIView {
    ...
    @objc func rotated(notification: NSNotification) {
        ...
        let loader = DASwiftLoader.sharedInstance
    ...
    class var sharedInstance: DASwiftLoader {
        struct Singleton {
            static let instance = DASwiftLoader(frame: CGRect(origin: CGPoint(x: 0,y: 0),size: CGSize(width: Config().size,height: Config().size)))
        }
